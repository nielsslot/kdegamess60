...
public:
    ...
    void setTextColor(const QBrush& brush) { m_brush = brush; }
...
{
    setDefaultTextColor(m_brush.color());
    p->save();
    ...
    : QGraphicsItem(parent), d(new KGamePopupItemPrivate)
{
    ...
    */
    ...
    d->m_brush = pal.brush( QPalette::ToolTipBase );
    d->m_textChildItem->setTextColor( pal.color( QPalette::ToolTipText ) );
...
{
    d->m_textChildItem->setTextColor(QBrush(color));
}

void KGamePopupItem::onLinkHovered(const QString& link)
{
    // Not so useful for S60 port ;)
    Q_UNUSED(link)
